Shader "Unlit/SimpleWater"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _Tint;
			uniform sampler2D _CameraDepthTexture; //creates a depth texture
			...
			{
				v2f o;
				float4 tex = tex2Dlod(_NoiseTex, float4(v.uv.xy, 0, 0));//creates more noise
				v.vertex.y += sin(_Time.z * _Speed + (v.vertex.x * v.vertex.z * _Amount * tex)) * _Height;//makes the water move
				o.vertex = UnityObjectToClipPos(v.vertex);
				o.uv = TRANSFORM_TEX(v.uv, _MainTex);
				o.scrPos = ComputeScreenPos(o.vertex); // gets the position on screen
